...
url_shorteners = ["bit.ly", "tinyurl.com", "t.co", "is.gd", "cutt.ly"]
suspicious_words = ["login", "verify", "secure", "account", "password"]

for url in suspicious_urls:
    if any(shortener in url for shortener in url_shorteners):
        suspicious_url_flags.append(f"URL shortener detected: {url}")
    if any(word in url for word in suspicious_words):
        suspicious_url_flags.append(f"Suspicious keyword in URL: {url}")
